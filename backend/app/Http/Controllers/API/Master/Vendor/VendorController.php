...
class VendorController extends ApiController
{
    ...
    public function getLazyLoadingList($limit, $offset) {
        $lazyData = DB::table('general.business_list')->selectRaw('Business_Number, business_fullname')->limit($limit)->offset($offset)->get();
        return response()->json($lazyData);
    }
    
    public function getsystemprofileorders() {
        $systemProfile = DB::table('general.systemprofileorders')
        ->select('FrmName as FRM_NAME', 'FrmClass as FRM_CLASS', 'GroupName as USER_GRP_IND', 'ORDERED as ORDERS')
        ->where('FrmName', 'Request for Payment')->orderBy('ORDERED', 'asc')
        ->get();

        for ($i = 0; $i < count($systemProfile); $i++) {
            $insertSystemProfileData[] = [
                'FRM_NAME'      => $systemProfile[$i]->FRM_NAME,
                'FRM_CLASS'     => $systemProfile[$i]->FRM_CLASS,
                'USER_GRP_IND'  => $systemProfile[$i]->USER_GRP_IND,
                'ORDERS'        => $systemProfile[$i]->ORDERS,
                'PROCESSID'        => 123,
                ...
                'DEPARTMENT'        => "IT",
                'RM_ID'        => "123",
                'REPORTING_MANAGER'        => "JR",
                'PROJECTID'        => "123",
                'PROJECT'        => "project",
                'COMPID'        => "123",
            ];
        }
        $output = DB:: table('general.actual_sign')->insert($insertSystemProfileData);
